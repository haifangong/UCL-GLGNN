    ...
    def __init__(self, num_layer, input_dim, emb_dim, out_dim, JK="last", drop_ratio=0, graph_pooling="attention",
        ...
        if self.concat_type == 'lstm':
            ...
            if self.feature_level == 'global-local':
                self.fc = nn.Sequential(
                    nn.Linear(4*self.emb_dim, self.emb_dim), nn.LeakyReLU(0.1), nn.Dropout(p=self.drop_ratio), nn.Linear(self.emb_dim, self.out_dim))
            else:
                self.fc = nn.Sequential(
                    nn.Linear(4*self.emb_dim, self.emb_dim), nn.LeakyReLU(0.1), nn.Dropout(p=self.drop_ratio), nn.Linear(self.emb_dim, self.out_dim))
    ...
    def forward(self, data, epoch=0):
        ...
        if self.dir:
            ...
            for i in range(node_rep_be.shape[0]):
                similarity_list.append(cosine_similarity(np.asarray(node_rep_be[i].cpu().detach()), np.asarray(node_rep_af[i].cpu().detach())))
            x = self.fc(x)
